...
const michelin = localFont({
  ...
  display: "swap",
  // Те саме, що в e-Ukraine вище. Спроба лишити preload лише тут (щоб
  // прибрати CLS 0,069 від підміни шрифта заголовка) нічого не дала:
  // next/font не вміє вмикати прелоад для однієї родини, якщо інша його
  // вимкнула — у <head> не з'явилось жодного preload-лінка. Обмін усе одно
  // на нашу користь: без прелоада LCP кращий на ~0,65 с, а CLS 0,069
  // лишається в «зеленій» зоні (поріг 0,1).
  preload: false,
});
